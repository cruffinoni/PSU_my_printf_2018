...
int type_int(va_list args, t_local_spe specifier_infos)
{
    ...
    int_recast_cnb(var, &count, 10, specifier_infos);
    ...
    count = 0;
    int_recast_pnb(var, &count, 10, specifier_infos);
    return (count + specifier_infos.precision);
}

int type_unsigned_int(va_list args, t_local_spe specifier_infos)
{
    uintptr_t var = va_arg(args, uintptr_t);
    int count = 0;

    uint_recast_cnb(var, &count, 10, specifier_infos);
    if (specifier_infos.precision > count)
        specifier_infos.precision -= count;
    else
        specifier_infos.precision = 0;
    print_char_ite(specifier_infos.precision, specifier_infos);
    count = 0;
    uint_recast_pnb(var, &count, 10, specifier_infos);
    return (count + specifier_infos.precision);
}